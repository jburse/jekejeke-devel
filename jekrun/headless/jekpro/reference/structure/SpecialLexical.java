...
public final class SpecialLexical extends AbstractSpecial {
    ...
    /**
     * <p>Match a term. As a side effect bindings arr established.</p>
    ...
    public static boolean matchTerm(Object alfa, Display d1,
        ...
        for (; ; ) {
            BindUniv b2;
            while (alfa instanceof SkelVar &&
                    (b2 = d1.bind[((SkelVar) alfa).id]).display != null) {
                alfa = b2.skel;
                d1 = b2.display;
            }
            if (beta instanceof SkelVar) {
                BindUniv b1;
                if ((b1 = d2.bind[((SkelVar) beta).id]).display != null) {
                    beta = b1.skel;
                    d2 = b1.display;
                    return SpecialLexical.equalTerm(alfa, d1, beta, d2);
                } else {
            ...
            int i = 0;
            for (; i < t1.length - 1; i++) {
                if (!matchTerm(t1[i], d1, t2[i], d2, en))
                    return false;
            }
            alfa = t1[i];
            beta = t2[i];
