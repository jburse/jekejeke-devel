 ...
 */
:- public ord_put/4.
ord_put(X, _, _, _) :- var(X),
   throw(error(instantiation_error, _)).
ord_put([K-_|M], J, U, R) :- J == K, !,
   R = [K-U|M].
ord_put([K-V|M], J, U, R) :- J @< K, !,
   R = [J-U, K-V|M].
ord_put([A|M], J, U, H) :- !,
   H = [A|N],
   ord_put(M, J, U, N).
 ...
 */
:- public ord_remove/3.
ord_remove(X, _, _) :- var(X),
   throw(error(instantiation_error, _)).
ord_remove([K-_|M], J, R) :- J == K, !,
   R = M.
ord_remove([K-V|M], J, R) :- J @< K, !,
   R = [K-V|M].
ord_remove([A|M], J, H) :- !,
   H = [A|N],
   ord_remove(M, J, N).
