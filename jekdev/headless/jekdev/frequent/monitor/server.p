...
start_monitor('') :- !.
start_monitor(A) :-
   thread_new((run_http(monitor/server, A), fail; true), Thread),
   thread_start(Thread).
...
dispatch(_, Path, Request, Session) :-
   ...
   monitor/desktop::dispatch(Path2, Request, Session).
dispatch(_, Path, Request, Session) :-
   sub_atom(Path, 0, Pos, '/mobile/'), !,
   Pos2 is Pos-1,
   sub_atom(Path, Pos2, _, 0, Path2),
   monitor/mobile::dispatch(Path2, Request, Session).
...
upgrade(_, Path, Request, Session) :-
   ...
   Pos2 is Pos-1,
   sub_atom(Path, Pos2, _, 0, Path2),
   monitor/view::upgrade(Path2, Request, Session).
