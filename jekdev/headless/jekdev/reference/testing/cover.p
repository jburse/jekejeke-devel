...
html_list_summary :-
   ...
   findall(W, (member(S, L), cover_source_view(S, _, _, W)), V),
   sys_sum_oknok(V, Z),
   uri_encode(D, DEnc),
...
html_list_element(R, L) :-
   ...
   write('  <td><a href="'), html_escape(RRNUriEnc),
...
html_list_member(R, S) :-
   ...
   html_functor_indicator(F, A, S), write('</a></td>'), nl,
   html_pairs_data(P),
   write('  </tr>'), nl,
   fail.
...
html_list_source(S, N, Z) :-
   ...
   html_functor_indicator(F, A, S), write('</h2>'), nl,
   write('<table class="rowtable">'), nl,
   write('  <tr class="headrow">'), nl,
...
html_list_predicate(F, A, S, Z) :-
   ...
   write('<a href="'), html_escape(RUUriEnc), write('">'),
   html_escape(NStr), write('</a></td>'), nl,
   html_pairs_data(P),
   write('  </tr>'), nl,
   fail.
