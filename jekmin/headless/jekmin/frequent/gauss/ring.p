 ...
 *
 * Our Gröbner Basis algorithm uses monomial lexical ordering. In this
 * ordering the head of a polynomial is not necessary the monomial
 * with the greatest degree. The greatest degree of a polynomial can be
 * determined by the predicate degree/2. Random polynomials over a given
 * set of variables can generated by the predicate randpoly/2.
 *
 * Warranty & Liability
...
/**
 * randpoly(L, P):
 * The predicate succeeds in P with a random polynomial over the variables L.
 */
:- public vector:randpoly/2.
vector:randpoly(vector, Y) :- !,
   random(11, R),
   user: -(R, 5, Y).
vector:randpoly(X, Z) :-
   X =.. [F, V|L],
