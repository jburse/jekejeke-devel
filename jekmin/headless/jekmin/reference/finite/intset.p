...
sys_hook_in(V, W) :-
   ...
   sys_freeze_var(V, R),
   sys_fresh_var(W, S),
   sys_assume_cont(<= +sys_var_in(R,S)).
sys_hook_in(V, T) :-
   integer(T), !,
   sys_freeze_var(V, R),
   sys_assume_cont(<= +sys_const_in(R,T)).
sys_hook_in(_, T) :-
...
sys_hook_set(V, W) :-
   ...
   sys_freeze_var(V, R),
   sys_fresh_var(W, S),
   sys_assume_cont(<= +sys_var_set(R,S)).
sys_hook_set(V, T) :-
   integer(T), !,
   sys_freeze_var(V, R),
   sys_assume_cont(<= +sys_const_set(R,T)).
sys_hook_set(_, T) :-
   throw(error(type_error(integer,T),_)).
...
sys_set_waits(X, V) <=
   + sys_set_ref(V, L, _, _),
   {member(_*X, L)}.
+ sys_set_agent(V, X, [A*X|B], S, T) <=
   = sys_set_ref(V, [A*X|B], S, T).
...
/* Degenerate Sets */
unit <=
   = sys_set_agent(_, _, _, [...], _), !.
zero <=
   = sys_set_agent(_, _, _, [], _), !.
+ sys_set_move(V) /\
+ (linform:sys_lin_agent(V, X, L, Y)) <=
   = sys_set_agent(V, X, L, [Y], _),
   {integer(Y)}, !.
/* GCD Normalisation */
zero <=
   = sys_set_agent(_, _, P, _, U),
   {sys_gcd_prod(P, G),
    \+ sys_div_range(U, G, _)}, !.
+ sys_set_agent(V, X, R, T, W) <=
   = sys_set_agent(V, X, P, S, U),
   {sys_gcd_prod(P, G)}, !,
   {sys_div_prod(P, G, R),
    sys_div_set(S, G, T),
    sys_div_range(U, G, W)}.
+ sys_set_agent(V, X, R, T, W) <=
   = sys_set_agent(V, X, [A*X|B], S, U),
   {A < 0}, !,
...
/* Simple Trigger */
+ sys_in(X, S, T) <=
   = sys_set_agent(V, X, [1*X], S, T), !,
   - sys_set_waits(X, V).
/* Set & Set Intersection */
+ sys_set_remove(J) /\
+ sys_set_agent(K, X, L, W, R) <=
   = sys_set_agent(J, X, L, S, P),
   - sys_set_agent(K, X, L, T, Q),
   {(  sys_inter_range(P, Q, R)
    -> sys_inter_set(S, T, W)
    ;  W = [],
       R = ...),
