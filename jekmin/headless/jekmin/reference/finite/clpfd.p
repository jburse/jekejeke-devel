...
sys_hook_mul(V, W) :- var(W), !,
   sys_freeze_var(V, R),
   sys_fresh_var(W, S),
   sys_assume_cont(post(sys_var_mul(R, S))).
sys_hook_mul(V, T) :- integer(T), !,
   sys_freeze_var(V, R),
...
post(sys_mulv_ord(X, Y, Z)) <=
   phaseout_posted(sys_mulv(X, Y, Z)), X @< Y, !.
post(sys_mulv_ord(X, Y, Z)) <=
   phaseout_posted(sys_mulv(Y, X, Z)).

/* Mul & Mul Intersection */
sys_melt_join(T, Z) <=
   phaseout_posted(sys_mulv_ord(X, Y, Z)), sys_mulv_ord(X, Y, T), !.
sys_melt_const(Z, C) <=
   phaseout_posted(sys_mulv_ord(X, Y, Z)), sys_mulc_ord(X, Y, C), !.
/* Union Find */
post(sys_mulv(T, Y, Z)) <=
   phaseout_posted(sys_mulv_ord(X, Y, Z)), sys_bound_var(X), sys_melt_var(X, H), var(H), !,
   sys_fresh_var(H, T).
post(sys_mulv(X, T, Z)) <=
   phaseout_posted(sys_mulv_ord(X, Y, Z)), sys_bound_var(Y), sys_melt_var(Y, H), var(H), !,
   sys_fresh_var(H, T).
post(sys_mulv(X, Y, T)) <=
   phaseout_posted(sys_mulv_ord(X, Y, Z)), sys_bound_var(Z), sys_melt_var(Z, H), var(H), !,
   sys_fresh_var(H, T).
/* Constant Elimination */
post(sys_lin(L, 0)) <=
   phaseout_posted(sys_mulv_ord(X, Y, Z)), sys_bound_var(X), sys_melt_var(X, C), integer(C), !,
   sys_make_prod(C, Y, [], H), sys_add_prod(H, [-1*Z], L).
post(sys_lin(L, 0)) <=
   phaseout_posted(sys_mulv_ord(X, Y, Z)), sys_bound_var(Y), sys_melt_var(Y, C), integer(C), !,
   sys_make_prod(C, X, [], H), sys_add_prod(H, [-1*Z], L).
post(sys_mulc_ord(X, Y, C)) <=
   phaseout_posted(sys_mulv_ord(X, Y, Z)), sys_bound_var(Z), sys_melt_var(Z, C), integer(C), !.
/* Hook Adding */
sys_melt_hook(X, sys_hook_mul), sys_melt_hook(Y, sys_hook_mul),
sys_melt_hook(Z, sys_hook_mul) <=
   posted(sys_mulv_ord(X, Y, Z)).
/* Set Diffusion, Directed, Bounds */
post(intset:sys_in(Z, [S], S)) <=
   posted(sys_mulv_ord(X, Y, Z)), Z @> Y, !,
   sys_bound_factor(X, R), sys_bound_factor(Y, T),
   sys_cross_range(R, T, S), S \== ... .
post(intset:sys_in(Y, L, S)) <=
   posted(sys_mulv_ord(X, Y, Z)),
   sys_bound_factor(Z, R), sys_bound_divisor(X, T),
   (  sys_slash_range(R, T, S)
   -> S \== ..., L = [S]
   ;  L = [], S = ...).
/* Variable Rename */
post(sys_mulv(T, Y, Z)) <=
   posted(sys_var_mul(X, T)), phaseout(sys_mulv_ord(X, Y, Z)).
post(sys_mulv(X, T, Z)) <=
   posted(sys_var_mul(Y, T)), phaseout(sys_mulv_ord(X, Y, Z)), Y \== X.
post(sys_mulv(X, Y, T)) <=
   posted(sys_var_mul(Z, T)), phaseout(sys_mulv_ord(X, Y, Z)), Z \== X, Z \== Y.
/* Constant Backpropagation */
post(sys_lin(L, 0)) <=
   posted(sys_const_mul(X, C)), phaseout(sys_mulv_ord(X, Y, Z)),
   sys_make_prod(C, Y, [], H), sys_add_prod(H, [-1*Z], L).
post(sys_lin(L, 0)) <=
   posted(sys_const_mul(Y, C)), phaseout(sys_mulv_ord(X, Y, Z)),
   Y \== X, sys_make_prod(C, X, [], H), sys_add_prod(H, [-1*Z], L).
post(sys_mulc_ord(X, Y, C)) <=
   posted(sys_const_mul(Z, C)), phaseout(sys_mulv_ord(X, Y, Z)),
   Z \== X, Z \== Y.
/* Set Update, Directed, Bounds */
post(intset:sys_in(Z, [S], S)) <=
   posted(intset:sys_in(X, _, R)), R \== ...,
   sys_mulv_ord(X, Y, Z),
   Z @> Y, sys_bound_factor(Y, T),
   sys_cross_range(R, T, S), S \== ... .
post(intset:sys_in(Z, [S], S)) <=
   posted(intset:sys_in(Y, _, R)), R \== ...,
   sys_mulv_ord(X, Y, Z),
   Z @> Y, sys_bound_factor(X, T),
   sys_cross_range(R, T, S), S \== ... .
post(intset:sys_in(Y, L, S)) <=
   posted(intset:sys_in(Z, _, R)), R \== ...,
   sys_mulv_ord(X, Y, Z),
   Z @=< Y, sys_bound_divisor(X, T),
   (  sys_slash_range(R, T, S)
   -> S \== ..., L = [S]
   ;  L = [], S = ...).
post(intset:sys_in(Y, L, S)) <=
...
/* Order Arguments */
post(sys_mulc_ord(X, Y, C)) <=
   phaseout_posted(sys_mulc(X, Y, C)), X @< Y, !.
post(sys_mulc_ord(X, Y, C)) <=
   phaseout_posted(sys_mulc(Y, X, C)).

/* Mul & Mul Intersection */
sys_melt_const(T, C) <=
   phaseout_posted(sys_mulc_ord(X, Y, C)), sys_mulv_ord(X, Y, T), !.
fail <=
   phaseout_posted(sys_mulc_ord(X, Y, C)), sys_mulc_ord(X, Y, D), C \== D, !.
true <=
   phaseout_posted(sys_mulc_ord(X, Y, C)), sys_mulc_ord(X, Y, C), !.
/* Union Find */
post(sys_mulc(Z, Y, C)) <=
   phaseout_posted(sys_mulc_ord(X, Y, C)), sys_bound_var(X), sys_melt_var(X, H), var(H), !,
   sys_fresh_var(H, Z).
post(sys_mulc(X, Z, C)) <=
   phaseout_posted(sys_mulc_ord(X, Y, C)), sys_bound_var(Y), sys_melt_var(Y, H), var(H), !,
   sys_fresh_var(H, Z).
/* Constant Elimination */
post(sys_lin(L, C)) <=
   phaseout_posted(sys_mulc_ord(X, Y, C)), sys_bound_var(X), sys_melt_var(X, D), integer(D), !,
   sys_make_prod(D, Y, [], L).
post(sys_lin(L, C)) <=
   phaseout_posted(sys_mulc_ord(X, Y, C)), sys_bound_var(Y), sys_melt_var(Y, D), integer(D), !,
   sys_make_prod(D, X, [], L).
/* Hook Adding */
sys_melt_hook(X, sys_hook_mul), sys_melt_hook(Y, sys_hook_mul) <=
...
/* Set Diffusion, Directed, Bounds */
post(intset:sys_in(Y, L, S)) <=
   posted(sys_mulc_ord(X, Y, C)), sys_bound_divisor(X, R),
   (  sys_slash_range(C, R, S)
   -> S \== ..., L = [S]
   ;  L = [], S = ...).
/* Variable Rename */
post(sys_mulc(Z, Y, C)) <=
   posted(sys_var_mul(X, Z)), phaseout(sys_mulc_ord(X, Y, C)).
post(sys_mulc(X, Z, C)) <=
   posted(sys_var_mul(Y, Z)), phaseout(sys_mulc_ord(X, Y, C)), Y \== X.
/* Constant Backpropagation */
post(sys_lin(L, C)) <=
   posted(sys_const_mul(X, S)), phaseout(sys_mulc_ord(X, Y, C)), sys_make_prod(S, Y, [], L).
post(sys_lin(L, C)) <=
   posted(sys_const_mul(Y, S)), phaseout(sys_mulc_ord(X, Y, C)), Y \== X, sys_make_prod(S, X, [], L).
/* Set Update, Directed, Bounds */
post(intset:sys_in(Y, L, S)) <=
   posted(intset:sys_in(X, _, R)),
   sys_mulc_ord(X, Y, C),
...
sys_hook_spez(V, W) :- var(W), !,
   sys_freeze_var(V, R),
   sys_fresh_var(W, S),
   sys_assume_cont(post(sys_var_spez(R, S))).
sys_hook_spez(V, T) :- integer(T), !,
   sys_freeze_var(V, R),
...
sys_melt_join(T, Z) <=
   phaseout_posted(sys_sqrv(X, Z)), sys_sqrv(X, T), !.
/* Union Find */
post(sys_sqrv(T, Y)) <=
   phaseout_posted(sys_sqrv(X, Y)), sys_bound_var(X), sys_melt_var(X, H), var(H), !,
   sys_fresh_var(H, T).
post(sys_sqrv(X, T)) <=
   phaseout_posted(sys_sqrv(X, Y)), sys_bound_var(Y), sys_melt_var(Y, H), var(H), !,
   sys_fresh_var(H, T).
/* Constant Elimination */
sys_melt_const(Y, D) <=
   phaseout_posted(sys_sqrv(X, Y)), sys_bound_var(X), sys_melt_var(X, C),
   integer(C), !, D is C*C.
post(sys_sqrc(X, C)) <=
   phaseout_posted(sys_sqrv(X, Y)), sys_bound_var(Y), sys_melt_var(Y, C),
   integer(C), !.
/* Hook Adding */
sys_melt_hook(X, sys_hook_spez), sys_melt_hook(Y, sys_hook_spez) <=
   posted(sys_sqrv(X, Y)).
/* Set Diffusion, Directed, Bounds */
post(intset:sys_in(X, L, S)) <=
   posted(sys_sqrv(X, Y)), X @>= Y, !, sys_bound_factor(Y, R),
   (  sys_root_range(R, S)
   -> S \== ..., L = [S]
   ;  L = [], S = ...).
post(intset:sys_in(Y, [S], S)) <=
   posted(sys_sqrv(X, Y)), sys_bound_divisor(X, R),
   sys_square_range(R, S).
/* Variable Rename */
post(sys_sqrv(T, Y)) <=
   posted(sys_var_spez(X, T)), phaseout(sys_sqrv(X, Y)).
post(sys_sqrv(X, T)) <=
   posted(sys_var_spez(Y, T)), phaseout(sys_sqrv(X, Y)), Y \== X.
/* Constant Backpropagation */
sys_melt_const(Y, D) <=
   posted(sys_const_spez(X, C)), phaseout(sys_sqrv(X, Y)), D is C*C.
post(sys_sqrc(X, C)) <=
   posted(sys_const_spez(Y, C)), phaseout(sys_sqrv(X, Y)), Y \== X.
/* Set Update, Directed, Bounds */
post(intset:sys_in(X, L, S)) <=
   posted(intset:sys_in(Y, _, R)), R \== ...,
   sys_sqrv(X, Y), X @>= Y,
...
fail <=
   phaseout_posted(sys_sqrc(_, C)), C < 0, !.
post(intset:sys_in(X, [H, D], H..D)) <=
   phaseout_posted(sys_sqrc(X, C)), sqrtrem(C, D, R), R =:= 0, H is -D, !.
fail <=
...
/* Union Find */
post(sys_impv(T, Y)) <=
   phaseout_posted(sys_impv(X, Y)), sys_bound_var(X), sys_melt_var(X, H), var(H), !,
   sys_fresh_var(H, T).
post(sys_impv(X, T)) <=
   phaseout_posted(sys_impv(X, Y)), sys_bound_var(Y), sys_melt_var(Y, H), var(H), !,
   sys_fresh_var(H, T).
/* Constant Elimination */
post(sys_lin(L, C)) <=
   phaseout_posted(sys_impv(X, Y)), sys_bound_var(X), sys_melt_var(X, C), integer(C), !,
   sys_make_prod(C, Y, [], L).
post(sys_lin(L, 0)) <=
   phaseout_posted(sys_impv(X, Y)), sys_bound_var(Y), sys_melt_var(Y, C), integer(C), !,
   D is C-1, sys_make_prod(D, X, [], L).
/* Hook Adding */
sys_melt_hook(X, sys_hook_spez), sys_melt_hook(Y, sys_hook_spez) <=
   posted(sys_impv(X, Y)).
/* Set Diffusion, Directed, Bounds */
post(intset:sys_in(X, [S], S)) <=
   posted(sys_impv(X, Y)), X @>= Y, !, sys_bound_factor(Y, R),
   sys_prem_range(R, S), S \== ... .
post(intset:sys_in(Y, [S], S)) <=
   posted(sys_impv(X, Y)), sys_bound_factor(X, R),
   sys_conc_range(R, S), S \== ... .
/* Variable Rename */
post(sys_impv(T, Y)) <=
   posted(sys_var_spez(X, T)), phaseout(sys_impv(X, Y)).
post(sys_impv(X, T)) <=
   posted(sys_var_spez(Y, T)), phaseout(sys_impv(X, Y)), Y \== X.
/* Constant Backpropagation */
post(sys_lin(L, C)) <=
   posted(sys_const_spez(X, C)), phaseout(sys_impv(X, Y)), sys_make_prod(C, Y, [], L).
post(sys_lin(L, 0)) <=
   posted(sys_const_spez(Y, C)), phaseout(sys_impv(X, Y)), Y \== X, D is C-1, sys_make_prod(D, X, [], L).
/* Set Update, Directed, Bounds */
post(intset:sys_in(X, [S], S)) <=
   posted(intset:sys_in(Y, _, R)), R \== ...,
   sys_impv(X, Y), X @>= Y,
...
sys_hook_abs(V, W) :- var(W), !,
   sys_freeze_var(V, R),
   sys_fresh_var(W, S),
   sys_assume_cont(post(sys_var_abs(R, S))).
sys_hook_abs(V, T) :- integer(T), !,
   sys_freeze_var(V, R),
...
sys_melt_join(T, Z) <=
   phaseout_posted(sys_absv(X, Z)), sys_absv(X, T), !.
/* Union Find */
post(sys_absv(T, Y)) <=
   phaseout_posted(sys_absv(X, Y)), sys_bound_var(X), sys_melt_var(X, H), var(H), !,
   sys_fresh_var(H, T).
post(sys_absv(X, T)) <=
   phaseout_posted(sys_absv(X, Y)), sys_bound_var(Y), sys_melt_var(Y, H), var(H), !,
   sys_fresh_var(H, T).
/* Constant Elimination */
sys_melt_const(Y, D) <=
   phaseout_posted(sys_absv(X, Y)), sys_bound_var(X), sys_melt_var(X, C),
   integer(C), !, D is abs(C).
post(sys_absc(X, C)) <=
   phaseout_posted(sys_absv(X, Y)), sys_bound_var(Y), sys_melt_var(Y, C),
   integer(C), !.
/* Hook Adding */
sys_melt_hook(X, sys_hook_abs), sys_melt_hook(Y, sys_hook_abs) <=
   posted(sys_absv(X, Y)).
/* Set Diffusion, Directed, Bounds */
post(intset:sys_in(X, L, S)) <=
   posted(sys_absv(X, Y)), X @>= Y, !, sys_bound_factor(Y, R),
   (  sys_invabs_range(R, S)
   -> S \== ..., L = [S]
   ;  L = [], S = ...).
post(intset:sys_in(Y, [S], S)) <=
   posted(sys_absv(X, Y)), sys_bound_divisor(X, R),
   sys_abs_range(R, S).
/* Variable Rename */
post(sys_absv(T, Y)) <=
   posted(sys_var_abs(X, T)), phaseout(sys_absv(X, Y)).
post(sys_absv(X, T)) <=
   posted(sys_var_abs(Y, T)), phaseout(sys_absv(X, Y)), Y \== X.
/* Constant Backpropagation */
sys_melt_const(Y, D) <=
   posted(sys_const_abs(X, C)), phaseout(sys_absv(X, Y)), D is abs(C).
post(sys_absc(X, C)) <=
   posted(sys_const_abs(Y, C)), phaseout(sys_absv(X, Y)), Y \== X.
/* Set Update, Directed, Bounds */
post(intset:sys_in(X, L, S)) <=
   posted(intset:sys_in(Y, _, R)), R \== ..., sys_absv(X, Y), X @>= Y,
   (  sys_invabs_range(R, S)
   -> S \== ..., L = [S]
   ;  L = [], S = ...).
post(intset:sys_in(Y, [S], S)) <=
   posted(intset:sys_in(X, _, R)), sys_absv(X, Y),
   X @< Y, sys_abs_range(R, S).
...
fail <=
   phaseout_posted(sys_absc(_, C)), C < 0, !.
post(intset:sys_in(X, [H, C], H..C)) <=
   phaseout_posted(sys_absc(X, C)), H is -C.
